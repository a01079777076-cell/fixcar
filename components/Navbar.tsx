...
import { Car, Heart, User, LogOut, ChevronDown } from "lucide-react";
  ...
  return (
    <>
      <style>{`
        ...
        .dropdown-menu { position:absolute; top:calc(100% + 8px); right:0; background:white; border:1px solid #ECEAE4; borderRadius:14px; padding:8px; minWidth:180px; boxShadow:0 8px 24px rgba(0,0,0,0.08); zIndex:200; }
        .dropdown-item { display:flex; alignItems:center; gap:10px; padding:10px 14px; borderRadius:10px; fontSize:14px; fontWeight:600; color:#555; cursor:pointer; transition:background 0.15s; border:none; background:transparent; width:100%; textAlign:left; fontFamily:'NanumSquareRound',sans-serif; }
      ...
      <nav style={{ background:"rgba(255,255,255,0.96)", backdropFilter:"blur(20px)", borderBottom:"1px solid #ECEAE4", height:"68px", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 52px", position:"sticky", top:0, zIndex:100 }}>
        ...
        <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
          {loading ? (
            ...
            <div className="dropdown">
              ...
              {showDropdown && (
                <div className="dropdown-menu" onClick={() => setShowDropdown(false)}>
                  <a href="/mypage" className="dropdown-item" style={{ textDecoration:"none" }}>
                    <User size={16} /> 마이페이지
                  </a>
                  <a href="/mypage" className="dropdown-item" style={{ textDecoration:"none" }}>
                    <Heart size={16} /> 찜 목록
                  ...
                  {user.role === "DEALER" && (
                    <a href="/dealer" className="dropdown-item" style={{ textDecoration:"none" }}>
                      <Car size={16} /> 딜러 대시보드
                    </a>
                  )}
                  {user.role === "ADMIN" && (
                    <a href="/admin" className="dropdown-item" style={{ textDecoration:"none" }}>
                      <Car size={16} /> 관리자
                    </a>
                  )}
            ...
            <>
              <a href="/login">
                <button style={{ background:"transparent", border:"2px solid #E0DDD7", padding:"9px 22px", borderRadius:"100px", fontSize:"14px", fontWeight:700, cursor:"pointer", fontFamily:"'NanumSquareRound',sans-serif" }}>로그인</button>
              </a>
              <a href="/quiz">
                <button style={{ background:"#FF3B1E", color:"white", border:"none", padding:"10px 22px", borderRadius:"100px", fontSize:"14px", fontWeight:800, cursor:"pointer", fontFamily:"'NanumSquareRound',sans-serif" }}>내 차 픽하기</button>
              </a>
            </>
          )}
        </div>
