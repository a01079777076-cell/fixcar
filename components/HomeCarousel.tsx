...
const SLIDE_PCT      = 100 / 3;  /* ≈33.333 — track 기준 1슬라이드 */
...
export default function HomeCarousel() {
  ...
  /* ── DOM 직접 스타일 제어 (React 배칭 완전 우회) ── */
  const setTrackTransform = useCallback((offsetPct: number, animated: boolean) => {
    const track = trackRef.current;
    if (!track) return;
    track.style.transition = animated ? `transform ${TRANSITION_MS}ms ${EASE}` : "none";
    track.style.transform  = `translateX(${-SLIDE_PCT + offsetPct}%)`;
  }, []);
  ...
  const goTo = useCallback((next: number, dir: "left" | "right") => {
    ...
    setTrackTransform(dir === "left" ? -SLIDE_PCT : SLIDE_PCT, true);
    setTimeout(() => {
      setCurrent(target);
  ...
  /* ── current 변경 시 위치 동기화 ── */
  useEffect(() => {
    if (!isSnapping.current) setTrackTransform(0, false);
  }, [current, setTrackTransform]);

  /* ── 컨테이너 폭 ── */
  const measure = () => {
    containerWidth.current = (trackRef.current?.offsetWidth || window.innerWidth * 3) / 3;
  };

  /* ── px → track % ── */
  const toTrackPct = (px: number) => (px / containerWidth.current) * SLIDE_PCT;

  /* ── 스냅: 드래그 위치에서 이어서 자연스럽게 ── */
  const snapTo = (dir: "next" | "prev" | "reset") => {
    ...
    isSnapping.current = true;
    /* 현재 DOM 위치(드래그 끝점)에서 → 목표까지 애니메이션 */
    setTrackTransform(dir === "next" ? -SLIDE_PCT : SLIDE_PCT, true);
    setTimeout(() => {
  ...
  const onMouseMove = (e: React.MouseEvent) => {
    ...
    const pct = toTrackPct(e.clientX - dragStartX.current);
    setTrackTransform(Math.max(-SLIDE_PCT, Math.min(SLIDE_PCT, pct)), false);
  };
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    const pct = toTrackPct(e.touches[0].clientX - touchStartX.current);
    setTrackTransform(Math.max(-SLIDE_PCT, Math.min(SLIDE_PCT, pct)), false);
  };
    ...
    <div style={{ position: "relative", overflow: "hidden" }}>
      <div
        ...
        style={{
          ...
          width: "300%",
          height: BANNER_H,
          transform: `translateX(${-SLIDE_PCT}%)`,
          cursor: "grab",
          userSelect: "none",
      ...
      <button onClick={prev} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", width: 44, height: 44, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10, backdropFilter: "blur(4px)" }}>‹</button>
      <button onClick={next} style={{ position: "absolute", right: 16, top: "50%", transform: "translateY(-50%)", width: 44, height: 44, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.15)", color: "white", fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10, backdropFilter: "blur(4px)" }}>›</button>
      ...
      <div style={{ position: "absolute", bottom: 16, left: 0, right: 0, zIndex: 10, display: "flex", alignItems: "center", justifyContent: "center", gap: 12, padding: "0 60px" }}>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {SLIDES.map((s, i) => (
            <button key={s.id} onClick={() => { setCurrent(i); setProgress(0); }} style={{ width: i === current ? 20 : 6, height: 6, borderRadius: 100, border: "none", background: i === current ? "white" : "rgba(255,255,255,0.4)", cursor: "pointer", transition: "all 0.3s", padding: 0, flexShrink: 0 }} />
          ))}
        </div>
