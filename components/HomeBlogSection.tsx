  ...
  useEffect(() => {
    fetch("/api/blog?limit=4")
      ...
      .then(d => {
        ...
        const nonPinned = arr.filter((p: BlogPost) => !(Array.isArray(p.tags) && p.tags.includes("대표글")));
          ...
          return (
            <Link key={post.id} href={`/blog/${post.id}`} style={{ textDecoration:"none" }}>
              <div style={{ background:"white", borderRadius:18, overflow:"hidden", cursor:"pointer", transition:"all 0.2s" }}>
                {/* 1:1 비율 이미지 컨테이너 */}
                <div style={{ position:"relative", paddingBottom:"100%", background:"#F0EEE9", overflow:"hidden" }}>
                ...
                <div style={{ padding:"14px 16px" }}>
                  <div style={{ fontSize:14, fontWeight:800, marginBottom:6, lineHeight:1.4, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>{post.title}</div>
                  <div style={{ fontSize:11, color:"#CCC" }}>{post.author?.name||"픽스카"} · {new Date(post.createdAt).toLocaleDateString("ko-KR")}</div>
                </div>
              </div>
            </Link>
          );
