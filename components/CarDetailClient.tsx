    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#F0EEE9" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto", padding: "20px 16px 100px" }}>
          ...
          {/* ═══ 사진 갤러리 (엔카 비율) ═══ */}
          <div style={{ display: "grid", gridTemplateColumns: images.length > 1 ? "1fr 220px" : "1fr", gap: 4, marginBottom: 12, borderRadius: 16, overflow: "hidden" }}>
            ...
            {sideImgs.length > 0 && <div style={{ display: "grid", gridTemplateRows: `repeat(${Math.min(sideImgs.length, 4)},1fr)`, gap: 4 }}>
              {sideImgs.map((img: string, i: number) => (
                <div key={i} onClick={() => setMainImg(i + 1)} style={{ flex: 1, overflow: "hidden", cursor: "pointer", position: "relative", minHeight: 0 }}>
                  ...
                  {i === sideImgs.length - 1 && images.length > 5 && <div onClick={e => { e.stopPropagation(); setShowAllPhotos(true); }} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "white", cursor: "pointer", fontSize: 12 }}><span style={{ fontWeight: 700 }}>+ 사진 모두보기</span></div>}
                </div>
              ))}
            </div>}
          </div>
