...
export default async function Home() {
  ...
  const cars = dbCars.length > 0 ? dbCars.map(car => ({
    ...
    { id:1, name:"현대 아반떼 CN7", year:"2021년식", mileage:"32,000km", fuel:"가솔린", price:1450, monthly:29, tags:["무사고","초보 추천"], badge:"이달의 PICK", query:"hyundai+elantra+white+sedan", images:[] },
    { id:2, name:"기아 K3",         year:"2020년식", mileage:"51,000km", fuel:"가솔린", price:1090, monthly:22, tags:["무사고","가성비"],   badge:"FIX 가성비",  query:"kia+k3+silver+sedan",          images:[] },
    { id:3, name:"현대 투싼 NX4",   year:"2022년식", mileage:"28,000km", fuel:"가솔린", price:2780, monthly:55, tags:["1인 오너","가족용"], badge:"가족 PICK",    query:"hyundai+tucson+suv+black",     images:[] },
  ];
  ...
  const jsonLdItemList = {
    ...
    "itemListElement": cars.map((car, i) => ({
      ...
      "item": {
        "@type": "Product",
        "name": car.name,
        "url": `https://www.fixcar.kr/cars/${car.id}`,
        "offers": {
          "@type": "Offer",
    ...
    <>
      ...
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .stats-section{padding:0 52px;}
        .promise-section{padding:96px 52px;}
        ...
        @media(max-width:1024px){
          ...
          .cta-cards{flex-direction:column!important;}
          .section-pad{padding-left:20px!important;padding-right:20px!important;}
          .stats-section{padding:0 20px!important;}
          .promise-section{padding:72px 20px!important;}
        ...
        @media(max-width:768px){
          ...
          .category-card{margin-left:12px!important;margin-right:12px!important;padding:18px 12px!important;gap:4px!important;}
          .category-item{min-width:60px!important;padding:6px 8px!important;}
          .category-icon{width:56px!important;height:56px!important;font-size:28px!important;}
          .hero-btns{flex-direction:column!important;align-items:stretch!important;}
          .hero-btns a,.hero-btns a button{width:100%!important;justify-content:center!important;}
        }
        ...
        @media(max-width:600px){
          ...
          .cta-grid{padding:32px 16px!important;}
          .stats-strip{grid-template-columns:1fr 1fr!important;}
          .stats-item{padding:14px 8px!important;}
          .stats-num{font-size:22px!important;}
          .promise-section{padding:56px 16px!important;}
          .promises-4{grid-template-columns:1fr!important;}
          .category-card{border-radius:16px!important;margin-left:8px!important;margin-right:8px!important;}
        }

        /* ══ 430px 소형 스마트폰 ══ */
        @media(max-width:430px){
          .section-pad{padding-left:12px!important;padding-right:12px!important;}
          .stats-section{padding:0 12px!important;}
        }
      ...
      <div style={{ minHeight:"100vh", background:"#F0EEE9" }}>
        ...
        <section style={{ position:"relative" }}>
          <div style={{
            ...
            <div style={{ position:"relative", zIndex:2 }}>
              <div style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(255,255,255,0.12)", backdropFilter:"blur(8px)", borderRadius:"100px", padding:"8px 20px", marginBottom:"24px" }}>
                ...
                <span style={{ fontSize:"clamp(11px,1.5vw,13px)", fontWeight:700, color:"rgba(255,255,255,0.9)", letterSpacing:"1px" }}>광주 No.1 AI 나에게 맞는 차량추천</span>
              </div>
              ...
              <p style={{ fontSize:"clamp(13px,2vw,18px)", color:"rgba(255,255,255,0.65)", fontWeight:400, lineHeight:1.8, marginBottom:"28px" }}>
              ...
              <div className="hero-btns" style={{ display:"flex", gap:"12px", flexWrap:"wrap", justifyContent:"center" }}>
                <a href="/cars"><button className="btn-red" style={{ padding:"16px 36px", fontSize:"clamp(14px,2vw,16px)" }}>매물 보러가기 <ArrowRight size={16}/></button></a>
              </div>
            </div>
          </div>
          ...
          <div className="category-card" style={{
            ...
            {[
              ...
              <a key={item.label} href={item.href} className="category-item" style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"8px", textDecoration:"none", padding:"8px 16px", borderRadius:"14px", transition:"all 0.2s", cursor:"pointer", minWidth:"80px" }}>
                <div className="category-icon" style={{ width:"72px", height:"72px", borderRadius:"18px", background:"transparent", border:"none", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"36px", transition:"all 0.2s", overflow:"hidden" }}>
                  {item.img
                    ? <Image src={item.img} alt={item.label} width={64} height={64} style={{ objectFit:"contain" }} />
                    : item.icon}
                </div>
        ...
        {/* ══ 실시간 통계 스트립 ══ */}
        <section className="stats-section" style={{ maxWidth:"1360px", margin:"24px auto 0" }}>
          <div className="stats-strip">
            <DailyCounter />
            {[["2,418+","등록 매물","#FF3B1E"],["98%","구매 만족도","#1847FF"],["4.9","앱 평점","#2D8A52"]].map(([num,label,color])=>(
              <div key={String(label)} className="stats-item" style={{ padding:"20px 16px", textAlign:"center", borderRight:"1px solid #F0EEE9" }}>
                <div className="stats-num" style={{ fontFamily:"'Bebas Neue',serif", fontSize:"28px", color:String(color), letterSpacing:"-1px" }}>{num}</div>
                <div style={{ fontSize:"clamp(10px,1.2vw,12px)", color:"#AAA", marginTop:"3px", fontWeight:400 }}>{label}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="section-pad" style={{ maxWidth:"1360px", margin:"0 auto 80px", paddingTop:0, paddingBottom:0 }}>
          <div className="pickfix-grid">
            <div className="pickfix-inner" style={{ background:"linear-gradient(135deg, #FF5A3C 0%, #E8290F 60%, #C41E08 100%)", padding:"64px 56px", position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", right:"-10px", bottom:"-30px", fontFamily:"'Bebas Neue',serif", fontSize:"clamp(120px,18vw,200px)", color:"rgba(255,255,255,0.2)", lineHeight:1, letterSpacing:"-5px", pointerEvents:"none" }}>PICK</div>
            ...
            <div className="pickfix-inner" style={{ background:"linear-gradient(135deg, #3060FF 0%, #1338E0 60%, #0A25B8 100%)", padding:"64px 56px", position:"relative", overflow:"hidden" }}>
              <div style={{ position:"absolute", right:"-10px", bottom:"-30px", fontFamily:"'Bebas Neue',serif", fontSize:"clamp(120px,18vw,200px)", color:"rgba(255,255,255,0.2)", lineHeight:1, letterSpacing:"-5px", pointerEvents:"none" }}>FIX</div>
        ...
        <section className="section-pad" style={{ maxWidth:"1360px", margin:"0 auto 80px", paddingTop:0, paddingBottom:0 }}>
          <div className="content-banners">
            {[
              { emoji:"",   bgImg:"/icon/main_ranking_button.png",   title:"자동차 랭킹",        subtitle:"세상 모든 차량 순위",         desc:"최고가·최저가·저주행·초보추천 랭킹표 전체",             href:"/ranking",   color:"#FF3B1E", bg:"#FFF0ED" },
              ...
              { emoji:"",   bgImg:"/icon/main_community_button.png",  title:"커뮤니티",           subtitle:"소식들 올릴 수 있는 곳",       desc:"구매후기·질문답변·정비정보·사진자랑 자유게시판",          href:"/community", color:"#CC6633", bg:"#FFF5ED" },
              { emoji:"",   bgImg:"/icon/main_blog_button.png",       title:"픽스카 블로그",      subtitle:"차량관리, 소모품, 필수꿀템",   desc:"차량 필수템부터, 관리 노하우 모음",                      href:"/blog",      color:"#2D8A52", bg:"#EAF6EF" },
              { emoji:"",   bgImg:"/icon/main_catalog_button.png",    title:"차량 카탈로그",      subtitle:"전세계 차량을 연도별로",        desc:"카탈로그 옵션, 출고가 다 모았어요. 자동차세·보험까지!",  href:"/catalog",   color:"#1847FF", bg:"#EEF2FF" },
            ].map(item=>(
              <a key={item.title} href={item.href}>
                <div className="content-banner-card" style={{
                  background: item.bgImg ? "transparent" : "white",
                  borderRadius:"20px", padding:"28px 30px", cursor:"pointer", height:"100%", minHeight:"clamp(160px,15vw,220px)",
                  position:"relative", overflow:"hidden",
                }}>
                  {item.bgImg && (
                    <>
                  ...
                  <div style={{ position:"relative", zIndex:2 }}>
                    ...
                    <div style={{ display:"inline-block", background: item.bgImg ? "rgba(255,255,255,0.2)" : item.bg, color: item.bgImg ? "white" : item.color, padding:"4px 14px", borderRadius:"100px", fontSize:"clamp(10px,1.2vw,12px)", fontWeight:800, marginBottom:"10px", backdropFilter: item.bgImg ? "blur(4px)" : "none" }}>{item.subtitle}</div>
                    <div style={{ fontSize:"clamp(16px,2vw,20px)", fontWeight:800, marginBottom:"8px", letterSpacing:"-0.5px", color: item.bgImg ? "white" : "#1A1A1A" }}>{item.title}</div>
        ...
        <section className="section-pad" style={{ maxWidth:"1360px", margin:"0 auto 80px", paddingTop:0, paddingBottom:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:"36px" }}>
            <div>
              ...
              <h2 style={{ fontSize:"clamp(22px,4vw,46px)", fontWeight:800, letterSpacing:"-1.5px", lineHeight:1.1 }}>오늘 픽스카 <span style={{ color:"#FF3B1E" }}>추천</span> 매물</h2>
            </div>
            <a href="/cars" style={{ display:"flex", alignItems:"center", gap:"6px", fontSize:"15px", fontWeight:700, color:"#888", whiteSpace:"nowrap" }}>전체 보기 <ChevronRight size={16}/></a>
          </div>
            ...
            {cars.map(car=>(
              <a key={car.id} href={`/cars/${car.id}`} className="car-card">
                ...
                <div style={{ padding:"18px 20px" }}>
                  <div style={{ fontSize:"clamp(14px,2vw,17px)", fontWeight:800, marginBottom:"4px" }}>{car.name}</div>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", paddingTop:"14px", borderTop:"1px solid #F0EEE9" }}>
                    <div>
                      <div style={{ fontSize:"clamp(20px,3vw,26px)", fontWeight:800, letterSpacing:"-0.5px" }}>{car.price.toLocaleString()}<span style={{ fontSize:"14px", fontWeight:700, color:"#AAA" }}>만원</span></div>
                      <div style={{ fontSize:"12px", color:"#1847FF", fontWeight:800, marginTop:"3px", display:"flex", alignItems:"center", gap:"3px" }}><Lock size={10}/> FIX · 월 {car.monthly}만원~</div>
                    </div>
                    <div style={{ background:"#1A1A1A", color:"#fff", padding:"10px 18px", borderRadius:"10px", fontSize:"13px", fontWeight:800, display:"flex", alignItems:"center", gap:"6px", flexShrink:0 }}>픽하기 <ArrowRight size={14}/></div>
                  </div>
                </div>
              </a>
            ))}
        ...
        {/* ══ FIX 약속 ══ */}
        <section className="promise-section" style={{ background:"#1A1A1A" }}>
          <div style={{ maxWidth:"1360px", margin:"0 auto" }}>
            <div className="promise-header">
              <div>
                <div style={{ fontSize:"12px", fontWeight:800, letterSpacing:"3px", color:"#7A9BFF", marginBottom:"12px" }}>FIX PROMISE</div>
                <h2 style={{ fontSize:"clamp(24px,4vw,52px)", fontWeight:800, letterSpacing:"-1.5px", color:"#fff", lineHeight:1.1 }}>픽스카가 지키는<br /><span style={{ color:"#7A9BFF" }}>FIX</span> 약속 4가지</h2>
              </div>
            ...
            <div className="promises-4">
              {promises.map(p=>(
                <div key={p.label} className="promise-card" style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.08)" }}>
                  ...
                  <div style={{ fontSize:"clamp(14px,2vw,16px)", fontWeight:800, color:"#fff", marginBottom:"10px" }}>{p.label}</div>
        ...
        {/* ══ 후기 ══ */}
        <section className="section-pad" style={{ maxWidth:"1360px", margin:"0 auto", paddingTop:"80px", paddingBottom:"80px" }}>
          <div style={{ marginBottom:"48px" }}>
            <div style={{ fontSize:"12px", fontWeight:800, letterSpacing:"3px", color:"#FF3B1E", marginBottom:"12px" }}>PICK STORIES</div>
            <h2 style={{ fontSize:"clamp(22px,4vw,46px)", fontWeight:800, letterSpacing:"-1.5px", lineHeight:1.1 }}>픽한 사람들 이야기</h2>
          </div>
          <div className="reviews-3">
            {reviews.map(r=>(
              <div key={r.name} className="review-card">
                ...
                <p style={{ fontSize:"clamp(13px,1.5vw,15px)", lineHeight:1.85, color:"#444", marginBottom:"22px", fontWeight:400 }}>&ldquo;{r.text}&rdquo;</p>
                <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                  <div style={{ width:"42px", height:"42px", background:r.color+"1A", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"16px", fontWeight:800, color:r.color, flexShrink:0 }}>{r.initial}</div>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontSize:"15px", fontWeight:800 }}>{r.name}</div>
                    <div style={{ fontSize:"12px", color:"#AAA", fontWeight:400 }}>{r.desc}</div>
                  </div>
                  <span style={{ background:r.color+"1A", color:r.color, padding:"4px 12px", borderRadius:"100px", fontSize:"12px", fontWeight:800, flexShrink:0 }}>{r.tag}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="cta-grid">
            ...
            <div className="cta-cards">
              <div style={{ background:"rgba(255,255,255,0.12)", border:"1px solid rgba(255,255,255,0.2)", borderRadius:"20px", padding:"24px 28px" }}>
                ...
                <a href="/quiz"><button style={{ background:"white", color:"#FF3B1E", border:"none", padding:"13px 28px", borderRadius:"10px", fontSize:"15px", fontWeight:800, cursor:"pointer", display:"flex", alignItems:"center", gap:"8px", width:"100%", justifyContent:"center", fontFamily:"'NanumSquareRound',sans-serif" }}>내 차 PICK하러 가기 <ArrowRight size={16}/></button></a>
              </div>
              <div style={{ background:"rgba(0,0,0,0.15)", border:"1px solid rgba(0,0,0,0.1)", borderRadius:"20px", padding:"24px 28px" }}>
                ...
                <a href="/cars"><button style={{ background:"transparent", color:"white", border:"2px solid rgba(255,255,255,0.5)", padding:"13px 28px", borderRadius:"10px", fontSize:"15px", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:"8px", width:"100%", justifyContent:"center", fontFamily:"'NanumSquareRound',sans-serif" }}><Lock size={16}/> FIX 가격 매물 보기</button></a>
              </div>
            </div>
          </div>
        </section>
          ...
          <div style={{ maxWidth:"1200px", margin:"0 auto", padding:"48px clamp(16px,4vw,32px) 40px" }}>
            ...
            <div style={{ display:"flex", gap:"12px", marginBottom:"32px", flexWrap:"wrap" }}>
              <a href="#" style={{ textDecoration:"none" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px", border:"1.5px solid #E0DDD7", borderRadius:"12px", padding:"12px 24px", background:"white", cursor:"pointer" }}>
                  ...
                  <div><div style={{ fontSize:"10px", color:"#999", fontWeight:400, lineHeight:1 }}>Download on the</div><div style={{ fontSize:"16px", fontWeight:800, color:"#333", lineHeight:1.3 }}>App Store</div></div>
                </div>
              </a>
              <a href="#" style={{ textDecoration:"none" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px", border:"1.5px solid #E0DDD7", borderRadius:"12px", padding:"12px 24px", background:"white", cursor:"pointer" }}>
                  ...
                  <div><div style={{ fontSize:"10px", color:"#999", fontWeight:400, lineHeight:1 }}>GET IT ON</div><div style={{ fontSize:"16px", fontWeight:800, color:"#333", lineHeight:1.3 }}>Google Play</div></div>
                </div>
              </a>
            </div>
            <div style={{ fontSize:"12px", color:"#CCC", fontWeight:400 }}>Copyright © FIXCAR. All Rights Reserved.</div>
          </div>
        </footer>
      </div>
    </>
