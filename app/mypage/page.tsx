...
import { Heart, MessageSquare, Car, Settings, ChevronRight, LogOut, Zap, Bell, Shield } from "lucide-react";

interface UserInfo { id:number; name:string; email:string; role:string; phone?:string; }
interface FavItem { id:number; car:{ id:number; name:string; brand:string; price:number; images:string[]; mileage:number; fuel:string } }
interface InqItem { id:number; message:string; status:string; reply?:string; createdAt:string; car:{ name:string; brand:string; price:number } }
...
export default function MyPage() {
  ...
  const [inqs, setInqs] = useState<InqItem[]>([]);
          ...
          <div style={{background:"linear-gradient(135deg,#1A1A1A,#333)",borderRadius:22,padding:"28px 26px",marginBottom:20,color:"white",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:16,position:"relative",zIndex:1}}>
              ...
              <div>
                ...
                {user.role!=="USER"&&<span style={{fontSize:10,background:"rgba(255,255,255,0.15)",padding:"2px 8px",borderRadius:4,fontWeight:700,marginTop:4,display:"inline-block"}}>{user.role}</span>}
              </div>
            </div>
            {/* 통계 */}
            <div style={{display:"flex",gap:24,marginTop:20,paddingTop:16,borderTop:"1px solid rgba(255,255,255,0.1)"}}>
              ...
              <div><div style={{fontSize:20,fontWeight:800}}>{inqs.filter(i=>i.status==="REPLIED").length}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.4)"}}>답변 완료</div></div>
            </div>
          </div>
            ...
            <>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:20}}>
                {MENU.map(m=>{
                  const Icon = m.icon;
                  const inner = (
                    <div style={{background:"white",borderRadius:16,padding:"22px 16px",textAlign:"center",cursor:"pointer",position:"relative"}}>
                      {m.count!==undefined&&m.count>0&&<div style={{position:"absolute",top:10,right:10,background:"#FF3B1E",color:"white",borderRadius:100,minWidth:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800}}>{m.count}</div>}
                      <Icon size={24} color={m.color} style={{marginBottom:8}}/>
                  ...
                  {favs.slice(0,3).map(f=>(
                    <Link key={f.id} href={`/cars/${f.car.id}`}>
                      <div style={{background:"white",borderRadius:14,padding:"14px 18px",marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div>
                          <div style={{fontSize:14,fontWeight:800}}>{f.car.name}</div>
                          <div style={{fontSize:11,color:"#AAA"}}>{f.car.brand} · {f.car.mileage?.toLocaleString()}km</div>
                        </div>
              ...
              {/* 로그아웃 */}
              <button onClick={handleLogout} style={{width:"100%",padding:"16px",background:"white",border:"1.5px solid #E8E6E1",borderRadius:14,fontSize:14,fontWeight:700,color:"#E24B4A",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontFamily:"'NanumSquareRound',sans-serif"}}>
                <LogOut size={16}/> 로그아웃
              </button>
            </>
          )}
          ...
          {tab==="favorites"&&(
            <div>
              ...
              {favs.length===0?<div style={{background:"white",borderRadius:16,padding:"48px 20px",textAlign:"center",color:"#CCC"}}>찜한 차량이 없어요</div>:(
                favs.map(f=>(
                  <Link key={f.id} href={`/cars/${f.car.id}`}>
                    <div style={{background:"white",borderRadius:14,padding:"16px 20px",marginBottom:8,display:"flex",gap:14,alignItems:"center"}}>
                      <div style={{width:70,height:52,borderRadius:10,background:"#F0EEE9",overflow:"hidden",flexShrink:0}}>
                        {f.car.images?.[0]?<img src={f.car.images[0]} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🚗</div>}
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:14,fontWeight:800}}>{f.car.name}</div>
                        <div style={{fontSize:11,color:"#AAA"}}>{f.car.mileage?.toLocaleString()}km · {f.car.fuel}</div>
                      </div>
                      <div style={{fontSize:16,fontWeight:800,color:"#FF3B1E"}}>{f.car.price?.toLocaleString()}<span style={{fontSize:10,color:"#AAA"}}>만</span></div>
                    </div>
                  </Link>
          ...
          {tab==="inquiries"&&(
            <div>
              ...
              {inqs.length===0?<div style={{background:"white",borderRadius:16,padding:"48px 20px",textAlign:"center",color:"#CCC"}}>문의 내역이 없어요</div>:(
                inqs.map(inq=>(
                  <div key={inq.id} style={{background:"white",borderRadius:14,padding:"18px 20px",marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      <span style={{fontSize:14,fontWeight:800}}>{inq.car.name}</span>
                      <span style={{fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:100,
                        background:inq.status==="REPLIED"?"#EAF6EF":"#FFF0ED",
                        color:inq.status==="REPLIED"?"#2D8A52":"#FF3B1E",
                      }}>{inq.status==="REPLIED"?"답변완료":"대기중"}</span>
                    </div>
                    <p style={{fontSize:13,color:"#666",lineHeight:1.7,marginBottom:inq.reply?8:0}}>{inq.message}</p>
                    {inq.reply&&(
                      <div style={{background:"#F8F7F4",borderRadius:10,padding:"12px 14px",marginTop:8}}>
                        <div style={{fontSize:11,fontWeight:800,color:"#1847FF",marginBottom:4}}>딜러 답변</div>
                        <p style={{fontSize:13,color:"#555",lineHeight:1.7}}>{inq.reply}</p>
                      </div>
                    )}
