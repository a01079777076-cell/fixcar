...
/* ═══ 자동 상태 라벨 ═══ */
function getAutoLabels(car: Car): { text: string; bg: string; color: string }[] {
  const labels: { text: string; bg: string; color: string }[] = [];
  /* 신규등록 (3일 이내) */
  if (car.createdAt) {
    const diff = (Date.now() - new Date(car.createdAt).getTime()) / 86400000;
    if (diff <= 3) labels.push({ text: "🆕 신규", bg: "#EEF5FF", color: "#0066FF" });
  }
  /* 무사고 */
  if (car.accident === false || car.isAccident === false) labels.push({ text: "무사고", bg: "#E8F8EF", color: "#2D8A52" });
  /* 1만km 이하 */
  if (car.mileage <= 10000) labels.push({ text: "거의신차", bg: "#FFF8E0", color: "#E8A020" });
  return labels;
}

/* ═══ 자동완성 후보 생성 ═══ */
function buildAutocomplete(): string[] {
  const items: string[] = [];
  for (const [brand, models] of Object.entries(BRAND_MODELS)) {
    items.push(brand);
    for (const m of models) { items.push(m); items.push(`${brand} ${m}`); }
  }
  return items;
}
const AUTOCOMPLETE_POOL = buildAutocomplete();
...
export default function CarsPage(){
  ...
  useEffect(()=>{
    fetch("/api/cars?limit=200").then(r=>r.json()).then(d=>{setCars(Array.isArray(d)?d:d.data||d.cars||[]);setLoading(false);}).catch(()=>{setCars([]);setLoading(false);});
  ...
  const handleSearch=(val:string)=>{
    ...
    if(val.length>=1){
      ...
      const matches=AUTOCOMPLETE_POOL.filter(i=>i.toLowerCase().includes(q)).slice(0,8);
      setAcItems(matches);
      setAcOpen(matches.length>0);
    } else { setAcOpen(false); }
  };

  const selectAc=(val:string)=>{ setSearch(val); setAcOpen(false); };
  ...
  const filtered=cars.filter(c=>{
    ...
    if(sort==="viewsHigh")return (b.views||0)-(a.views||0);
    return 0;
  });
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          {BRANDS.map(b=>(
            <div key={b}>
              <button onClick={()=>{setBrand(b);setModel("전체");}} style={{padding:"9px 0",textAlign:"left",border:"none",background:"transparent",fontSize:13,fontWeight:brand===b?800:400,color:brand===b?"#FF3B1E":"#666",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",width:"100%",borderBottom:"1px solid #F0EEE9",display:"flex",justifyContent:"space-between"}}>
                <span>{b}</span>
      ...
      <div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          {YEARS.map(y=>(<button key={y.l} onClick={()=>setYearR(y)} style={{padding:"8px 0",textAlign:"left",border:"none",background:"transparent",fontSize:13,fontWeight:yearR.l===y.l?800:400,color:yearR.l===y.l?"#FF3B1E":"#666",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",borderBottom:"1px solid #F0EEE9"}}>{y.l}</button>))}
        </div>
      </div>
      <div>
        <div style={{fontSize:13,fontWeight:800,marginBottom:10,color:"#1A1A1A"}}>주행거리</div>
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          {MILES.map(m=>(<button key={m.l} onClick={()=>setMileR(m)} style={{padding:"8px 0",textAlign:"left",border:"none",background:"transparent",fontSize:13,fontWeight:mileR.l===m.l?800:400,color:mileR.l===m.l?"#FF3B1E":"#666",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",borderBottom:"1px solid #F0EEE9"}}>{m.l}</button>))}
        </div>
      </div>
      <div>
        <div style={{fontSize:13,fontWeight:800,marginBottom:10,color:"#1A1A1A"}}>가격</div>
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          {PRICES.map(p=>(<button key={p.l} onClick={()=>setPriceR(p)} style={{padding:"8px 0",textAlign:"left",border:"none",background:"transparent",fontSize:13,fontWeight:priceR.l===p.l?800:400,color:priceR.l===p.l?"#FF3B1E":"#666",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",borderBottom:"1px solid #F0EEE9"}}>{p.l}</button>))}
        </div>
      </div>
      <div>
        <div style={{fontSize:13,fontWeight:800,marginBottom:10,color:"#1A1A1A"}}>연료</div>
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {FUELS.map(f=>(<button key={f} onClick={()=>setFuel(f)} style={{padding:"7px 14px",borderRadius:100,border:fuel===f?"2px solid #FF3B1E":"1.5px solid #E0DDD7",background:fuel===f?"#FFF0ED":"white",color:fuel===f?"#FF3B1E":"#777",fontSize:12,fontWeight:fuel===f?800:600,cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif"}}>{f}</button>))}
        </div>
      </div>
      {activeF>0&&(<button onClick={resetF} style={{padding:"10px",background:"#F0EEE9",color:"#888",border:"none",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",textAlign:"center"}}>🔄 선택 초기화</button>)}
    </div>
  );
  ...
  return(
    <>
        ...
        <div style={{background:"#1A1A1A",padding:"32px 24px 24px"}}>
          <div style={{maxWidth:1280,margin:"0 auto"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
              ...
              {/* ═══ 검색 + 자동완성 ═══ */}
              <div ref={acRef} style={{position:"relative",width:320,maxWidth:"100%"}}>
                <span style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",fontSize:15,opacity:0.5}}>🔍</span>
                <input type="text" placeholder="차량명, 브랜드 검색" value={search} onChange={e=>handleSearch(e.target.value)} onFocus={()=>{if(search.length>=1&&acItems.length>0)setAcOpen(true);}}
                ...
                {acOpen&&acItems.length>0&&(
                  <div style={{position:"absolute",top:"100%",left:0,right:0,background:"white",borderRadius:12,boxShadow:"0 8px 32px rgba(0,0,0,0.15)",maxHeight:280,overflowY:"auto",zIndex:100,marginTop:4}}>
                    {acItems.map((item,i)=>(
                      <button key={i} onClick={()=>selectAc(item)} style={{width:"100%",padding:"11px 16px",border:"none",background:"transparent",textAlign:"left",cursor:"pointer",borderBottom:"1px solid #F0EEE9",fontSize:13,fontWeight:600,color:"#333",fontFamily:"'NanumSquareRound',sans-serif",display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontSize:12,opacity:0.4}}>🔍</span> {item}
                      </button>
                    ))}
                  </div>
                )}
        ...
        <div style={{maxWidth:1280,margin:"0 auto",padding:"20px 16px 120px",display:"flex",gap:20,alignItems:"flex-start"}}>
          <div className="pc-sidebar" style={{width:220,flexShrink:0,position:"sticky",top:80}}>
            <div style={{background:"white",borderRadius:18,padding:"20px 18px",boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                <div style={{fontSize:15,fontWeight:800}}>검색 결과 <span style={{color:"#FF3B1E"}}>{filtered.length}</span>대</div>
          ...
          <div className="main-area" style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
              <div style={{display:"flex",gap:6,fontSize:13,overflowX:"auto"}}>
                {[["latest","최근 등록순"],["priceLow","낮은 가격순"],["priceHigh","높은 가격순"],["mileageLow","낮은 주행거리"],["yearNew","최근연식순"],["viewsHigh","조회순"]].map(([k,l])=>(
                  <button key={k} onClick={()=>setSort(k)} style={{border:"none",background:"transparent",fontSize:13,fontWeight:sort===k?800:400,color:sort===k?"#1A1A1A":"#AAA",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif",borderBottom:sort===k?"2px solid #FF3B1E":"2px solid transparent",paddingBottom:4,whiteSpace:"nowrap"}}>{l}</button>
                ))}
              </div>
              <div style={{display:"flex",gap:4}}>
                {(["list","grid"] as const).map(v=>(
                  <button key={v} onClick={()=>setView(v)} style={{width:32,height:32,borderRadius:8,border:"1.5px solid #E0DDD7",background:view===v?"#1A1A1A":"white",color:view===v?"white":"#AAA",fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{v==="list"?"☰":"⊞"}</button>
                ))}
              </div>
            </div>
                      ...
                      <div className="list-item" style={{display:"flex",gap:16,padding:"18px 20px",borderBottom:idx<filtered.length-1?"1px solid #F0EEE9":"none",alignItems:"flex-start",cursor:"pointer"}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                            ...
                            {tags.slice(0,3).map(t=>(<span key={t} style={{background:"#F8F7F4",color:"#888",padding:"3px 10px",borderRadius:100,fontSize:11,fontWeight:600}}>{t}</span>))}
                          </div>
      ...
      <button onClick={()=>setShowCalc(true)} style={{position:"fixed",bottom:90,right:20,zIndex:1000,width:56,height:56,borderRadius:"50%",background:"#1847FF",color:"white",border:"none",boxShadow:"0 4px 20px rgba(24,71,255,0.4)",fontSize:22,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>🧮</button>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setShowCalc(false)}>
          <div onClick={e=>e.stopPropagation()} style={{background:"white",borderRadius:22,padding:"28px 24px",maxWidth:420,width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
              <div><label style={{fontSize:12,fontWeight:700,display:"block",marginBottom:4}}>차량 가격 (만원)</label><input type="number" value={calcPrice} onChange={e=>setCalcPrice(e.target.value)} style={{width:"100%",padding:"12px 14px",border:"1.5px solid #E0DDD7",borderRadius:10,fontSize:15,fontFamily:"'NanumSquareRound',sans-serif"}}/></div>
              <div><label style={{fontSize:12,fontWeight:700,display:"block",marginBottom:4}}>선수금 (만원)</label><input type="number" value={calcDown} onChange={e=>setCalcDown(e.target.value)} style={{width:"100%",padding:"12px 14px",border:"1.5px solid #E0DDD7",borderRadius:10,fontSize:15,fontFamily:"'NanumSquareRound',sans-serif"}}/></div>
              <div><label style={{fontSize:12,fontWeight:700,display:"block",marginBottom:4}}>할부 기간</label><select value={calcMonths} onChange={e=>setCalcMonths(e.target.value)} style={{width:"100%",padding:"12px 14px",border:"1.5px solid #E0DDD7",borderRadius:10,fontSize:15,fontFamily:"'NanumSquareRound',sans-serif"}}>{[12,24,36,48,60,72].map(m=><option key={m} value={m}>{m}개월</option>)}</select></div>
              <div><label style={{fontSize:12,fontWeight:700,display:"block",marginBottom:4}}>금리 (%)</label><input type="number" step="0.1" value={calcRate} onChange={e=>setCalcRate(e.target.value)} style={{width:"100%",padding:"12px 14px",border:"1.5px solid #E0DDD7",borderRadius:10,fontSize:15,fontFamily:"'NanumSquareRound',sans-serif"}}/></div>
            </div>
            {(()=>{
      ...
      <style>{`@media(max-width:1024px){.mob-filter-wrap{display:block!important;}}`}</style>
    </>
  );
}
