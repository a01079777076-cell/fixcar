...
export default function CarDetailClient() {
  ...
  useEffect(() => {
    fetch(`/api/cars/${id}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.success && d.data) { setCar(d.data); saveRecentCar(d.data); }
        else if (d.id) { setCar(d); saveRecentCar(d); }
        setLoading(false);
      })
  ...
  const sendInquiry = async () => {
    ...
    try {
      const res = await fetch("/api/inquiries", {
        ...
        body: JSON.stringify({ carId: Number(id), message: inquiryMsg }),
      });
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "#F0EEE9" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px 100px" }}>
          <Link href="/cars" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700, color: "#888", marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 24 }}>
              ...
              {images.length > 1 && (
                <div style={{ display: "flex", gap: 8, overflowX: "auto" }}>
                  {images.map((img: string, i: number) => (
                    <button key={i} onClick={() => setMainImg(i)} style={{ width: 72, height: 54, borderRadius: 10, overflow: "hidden", border: i === mainImg ? "3px solid #FF3B1E" : "2px solid transparent", cursor: "pointer", flexShrink: 0, padding: 0, background: "none" }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                ...
                {!car.accident && (
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 100, background: "#EAF6EF", color: "#2D8A52" }}>무사고</span>
                )}
                {car.inspected && (
                  <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 100, background: "#F0FAF4", color: "#2D8A52", display: "flex", alignItems: "center", gap: 3 }}>
                    <Award size={10} />검수완료
                  </span>
                )}
              </div>
              ...
              <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>
                {car.year}년 · {car.mileage?.toLocaleString()}km · {car.fuel} · {car.color} · {car.transmission}
              </div>
              ...
              {dealer && (
                <Link href={`/shops/${dealer.id}`}>
                  <div style={{ background: "white", borderRadius: 18, padding: "18px 20px", marginBottom: 16, display: "flex", alignItems: "center", gap: 14, cursor: "pointer", border: "1px solid #E8E6E1", transition: "all 0.15s" }}>
                    <div style={{ width: 44, height: 44, borderRadius: "50%", background: isVerified ? "linear-gradient(135deg,#0055FF,#003399)" : "#E8E6E1", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: isVerified ? "white" : "#888", flexShrink: 0 }}>
                      {(dealer.shopName || "D")[0]}
                    </div>
                    <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <button onClick={toggleFav} style={{ flex: 1, padding: "16px", background: isFav ? "#FFF0ED" : "white", border: isFav ? "2px solid #FF3B1E" : "1.5px solid #E0DDD7", borderRadius: 14, fontSize: 15, fontWeight: 800, color: isFav ? "#FF3B1E" : "#888", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'NanumSquareRound',sans-serif" }}>
                  <Heart size={18} fill={isFav ? "#FF3B1E" : "none"} />{isFav ? "찜 완료" : "찜하기"}
                </button>
                <button onClick={() => setShowInquiry(!showInquiry)} style={{ flex: 1, padding: "16px", background: "#FF3B1E", color: "white", border: "none", borderRadius: 14, fontSize: 15, fontWeight: 800, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'NanumSquareRound',sans-serif" }}>
          ...
          {showInquiry && (
            <div style={{ background: "white", borderRadius: 20, padding: "24px", marginTop: 20 }}>
              ...
              <textarea
                ...
                placeholder="궁금한 점을 자유롭게 작성해주세요 (차량 상태, 시승 가능 여부, 할부 조건 등)"
                maxLength={2000}
                style={{ width: "100%", padding: "14px 16px", border: "1.5px solid #E0DDD7", borderRadius: 12, fontSize: 14, fontFamily: "'NanumSquareRound',sans-serif", resize: "none", lineHeight: 1.8 }}
              />
