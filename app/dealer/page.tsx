...
import { Car, MessageSquare, Plus, Eye, TrendingUp, ChevronRight, Settings, Pencil } from "lucide-react";
      ...
      <div style={{minHeight:"100vh",background:"#F0F4FF"}}>
        <div style={{background:"white",borderBottom:"1px solid #DDEEFF",padding:"20px 24px"}}>
          <div style={{maxWidth:1000,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:20,fontWeight:800}}>🏪 딜러 대시보드</div><div style={{fontSize:13,color:"#888"}}>{user?.name}님, 오늘도 좋은 하루 되세요!</div></div>
        ...
        <div style={{maxWidth:1000,margin:"0 auto",padding:"20px 16px 80px"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:24}}>
            {[{icon:Car,label:"판매 중",value:activeCars.length,color:"#0066FF"},{icon:Eye,label:"검수 대기",value:reviewingCars.length,color:"#E8A020"},{icon:MessageSquare,label:"답변 대기",value:pendingInq.length,color:"#FF3B1E"},{icon:TrendingUp,label:"판매 완료",value:soldCars.length,color:"#2D8A52"}].map(s=>{const Icon=s.icon;return(<div key={s.label} style={{background:"white",borderRadius:16,padding:"22px 20px"}}><Icon size={20} color={s.color} style={{marginBottom:8}}/><div style={{fontSize:28,fontWeight:800,color:s.color}}>{s.value}</div><div style={{fontSize:12,color:"#AAA"}}>{s.label}</div></div>);})}
          </div>

          {pendingInq.length>0&&<Link href="/dealer/inquiries"><div style={{background:"#FFF0ED",borderRadius:16,padding:"18px 22px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",border:"1px solid #FFB8A8",cursor:"pointer"}}><div><div style={{fontSize:14,fontWeight:800,color:"#FF3B1E"}}>📨 답변 대기 문의 {pendingInq.length}건</div><div style={{fontSize:12,color:"#CC6633",marginTop:2}}>빠른 답변이 계약률을 높여요!</div></div><ChevronRight size={18} color="#FF3B1E"/></div></Link>}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:24}}>
            {[{label:"문의 관리",href:"/dealer/inquiries",icon:MessageSquare,color:"#0066FF"},{label:"매물 등록",href:"/dealer/cars/new",icon:Plus,color:"#2D8A52"},{label:"설정",href:"/settings",icon:Settings,color:"#888"}].map(m=>{const Icon=m.icon;return <Link key={m.label} href={m.href}><div style={{background:"white",borderRadius:14,padding:"18px",textAlign:"center",cursor:"pointer"}}><Icon size={22} color={m.color} style={{marginBottom:6}}/><div style={{fontSize:13,fontWeight:700}}>{m.label}</div></div></Link>;})}
          </div>
        </div>
      </div>
    </>
