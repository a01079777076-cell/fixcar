                ...
                : cars.sort((a,b)=>{const order:Record<string,number>={"REVIEWING":0,"AVAILABLE":1,"RESERVED":2,"SOLD":3};return (order[a.status]??9)-(order[b.status]??9);}).map(car=>{
                  ...
                  <div key={car.id} style={{background:"white",borderRadius:16,padding:"18px 22px",marginBottom:10,border:car.status==="REVIEWING"?(warnings.length>0?"2px solid #E24B4A":"2px solid #FFE4DE"):"1px solid #F0EEE9"}}>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      ...
                      <a href={`/dealer/cars/new?edit=${car.id}`}><button style={{padding:"8px 16px",background:"#0066FF",border:"none",borderRadius:10,fontSize:12,fontWeight:700,color:"white",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif"}}>✏️ 수정</button></a>
                      <a href={`/admin/cars/${car.id}/edit`}><button style={{padding:"8px 16px",background:(!car.images||car.images.length===0)?"#E8A020":"#EEF5FF",border:(!car.images||car.images.length===0)?"none":"1.5px solid #0066FF",borderRadius:10,fontSize:12,fontWeight:700,color:(!car.images||car.images.length===0)?"white":"#0066FF",cursor:"pointer",fontFamily:"'NanumSquareRound',sans-serif"}}>📷 사진{(!car.images||car.images.length===0)?"등록":"관리"}</button></a>
